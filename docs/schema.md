...
실데이터로 확인한 것: 위비티 상세는 분야(광고/마케팅, 웹/모바일/IT, 과학/공학, 예체능/미술/음악 등)와
응모대상(대학생/일반인/청소년 등)을 자체 분류로 이미 제공한다. 그리고 첫 페이지에는 "제한없음/누구나"가 흔하다.
그래서 전부 LLM에 맡기지 않는다. 구조로 오는 값은 표로 매핑하고, LLM은 자유문장으로 된 참가조건만 맡는다.
이게 정확도와 비용을 같이 낮춘다.
...
위비티 분야 태그도 이 표로 매핑한다 (웹/모바일/IT -> IT, 과학/공학 -> 과학, 예체능/미술/음악 -> 예술,
광고/마케팅 -> 경영 등). 위비티가 구조로 주는 값이라 LLM 없이 매핑한다.
