...
소스는 값을 두 방식으로 준다. API(온통청년 등)나 일부 사이트는 분야, 대상, 지역을 자체 분류로 이미
...
만 나이 조건("만 15~34세 청년" 등)은 사용자층이 대학생(어린 편)이라 늘 만족이라 사람을 거를 일이 없다.
그래서 나이는 수집·파싱·매칭 어디서도 다루지 않는다(제품 결정). 단, 초·중·고생 전용 공고는 나이 매칭이
아니라 "수집 제외 필터"로 애초에 안 가져온다 - 이건 별개라 유지한다.

### 거의 가능(조건 1개 차이) 기준

결과를 가능/확인 필요/거의 가능으로 나눌 때, 거의 가능은 딱 한 조건만 어긋난 경우만 보여준다.
- 학년: 1급간(1학년) 차이까지 근접으로 본다. 예: 3학년 대상인데 나는 2학년.
- 나머지(전공, 지역, 학점, 소득 등): 맞으면 통과, 아니면 어긋남 하나로 센다. 학점 0.1 차이 같은 미세 근접은 안 따지고 맞다/틀리다로만 본다.
...
create table postings (
  ...
  deadline date,
  posted_at date,
  eligibility jsonb,                  -- { grades, majors[], regions[], enrollment[], incomeMax, gpaMin, found_in_text, confidence }
