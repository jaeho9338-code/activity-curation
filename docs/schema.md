...
단순화 원칙: 개인 4주 프로젝트라 테이블을 최소로 둔다. 실제로는 수집 원문(raw_postings)과 완성본(postings) 둘이면 시작한다. 아래 sources·parse_logs·profiles는 나중에 필요해지면 붙이고, 처음부터 다 만들 필요 없다. 매칭은 완성본을 서버 메모리에 올려 필터링한다(복잡한 JSONB 쿼리 없이 match.js 재활용).
