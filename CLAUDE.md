...
- LLM 비용은 신규나 변경분만 파싱하고, API가 구조로 주는 값(지역, 연령, 소득 등)은 규칙으로 매핑해 호출을 줄여 통제한다. 크롤링한 자유 문장만 저렴한 모델로 공고당 분류와 추출 2콜. 하루 신규 수백 건이면 대략 하루 1달러 안쪽으로 본다. 정확한 값은 키가 생기면 실측한다.
