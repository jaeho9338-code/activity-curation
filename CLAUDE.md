...
- 매칭은 서버에서 한다. 조회하면 서버가 완성본을 프로필과 대조해 가능/거의 가능/확인 필요로 나눠 돌려준다. 공고가 수천 건 규모라 서버가 메모리에 올려 필터링해도 충분하다. 복잡한 Postgres JSONB 쿼리는 안 쓴다(오버엔지니어링). 프로토타입 match.js 로직을 서버에서 그대로 재활용한다.
