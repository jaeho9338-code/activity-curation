-- 대외활동 큐레이션 - Supabase 초기 세팅 (한 번에 실행)
-- Supabase 프로젝트 > 왼쪽 SQL Editor > New query 에 이 파일 전체를 붙여넣고 Run 하면 된다.
-- 테이블 2개를 만들고, 공고 14건을 넣는다.
...
create table postings (
  ...
  posted_at date,
  eligibility jsonb,
  parse_status text default 'curated',
  created_at timestamptz default now()
);

-- 화면이 anon key로 읽을 수 있게 RLS 열기(읽기 전용 공개)
alter table postings enable row level security;
create policy "postings 읽기 공개" on postings for select using (true);
